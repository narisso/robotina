...
def optimal_path(file_name):
	...
	for i in range(0, MAX_ROW*MAX_COL):		
		...
		data = map(int, data)
		walls.append(data)
		walls2[(data[0],data[1])] = (data[2],data[3],data[4], data[5])
	...
	for i in range(0, MAX_START):
		...
		graph.add_node((int(data[0]),int(data[1]),orientation))
